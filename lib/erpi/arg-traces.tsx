...
function Trace({path}:{path:string}){
 ...
 return <aside className="arg-trace" aria-label="Indice du jeu d’exploration"><div className="arg-mark"><span>⟡</span><small>{item.id} / {item.label}</small></div>
 {!found&&<div className={'arg-object arg-object-'+item.id}>
 {path==='/'&&<><button className="arg-seal" aria-label="Examiner la marque" onClick={()=>{Count(count+1);if(count>=2)discover()}}>III</button><p>{count?'Encore un contact…':'Trois contacts réveillent une trace.'}</p></>}
 {path==='/dossiers'&&<button className="arg-fold" onClick={discover}><span>Pièce jointe oubliée</span><small>Soulever le coin</small></button>}
 {path==='/videos'&&<><label htmlFor="arg-frequency">Fréquence <output>{tune.toFixed(1)}</output></label><input id="arg-frequency" type="range" min="82" max="92" step="0.1" value={tune} onChange={e=>{const value=Number(e.target.value);Tune(value);if(Math.abs(value-87.4)<.05)discover()}}/><p>Une annotation sur le récepteur : 87,4.</p></>}
 {path==='/actualites'&&<button className="arg-mirror" onClick={discover}><span>REGARDEZ AUTREMENT</span><small>Retourner la note</small></button>}
 {path==='/carte'&&<button className="arg-coordinate" onClick={discover}>00° 00′ 00″<small>Examiner ce point</small></button>}
 ...
 {found&&<div className="arg-found" role="status"><strong>{item.word}</strong><p>{item.note}</p></div>}
 <button className="arg-notebook-link" onClick={()=>dialog.current?.showModal()}>Carnet des traces {collection.length>0&&`· ${collection.length}/6`}</button>
